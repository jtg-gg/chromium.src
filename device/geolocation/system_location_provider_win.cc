...
void SystemLocationProviderWin::NotifyNewGeoposition(Geoposition position) {
...
static void getGeopositionData(CComPtr<ILatLongReport>& spLatLongReport, Geoposition& position) {
...
const Geoposition& SystemLocationProviderWin::GetPosition() {
  ...
  if (SUCCEEDED(hr)) {
    ...
    last_position_.error_code = hr == E_ACCESSDENIED ? Geoposition::ERROR_CODE_PERMISSION_DENIED : Geoposition::ERROR_CODE_POSITION_UNAVAILABLE;
  }
