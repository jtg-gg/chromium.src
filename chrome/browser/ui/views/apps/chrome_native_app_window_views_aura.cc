...
Browser::DownloadCloseType ChromeNativeAppWindowViewsAura::OkToCloseWithInProgressDownloads(
  ...
  *num_downloads_blocking = 0;
  ...
  if (notClosingWindow > 1)
    return Browser::DownloadCloseType::kOk;   // Not the last window; can definitely close.

  int total_download_count =
    DownloadCoreService::NonMaliciousDownloadCountAllProfiles();
  if (total_download_count == 0)
    return Browser::DownloadCloseType::kOk;   // No downloads; can definitely close.

  *num_downloads_blocking = total_download_count;
  return Browser::DownloadCloseType::kBrowserShutdown;
}
...
bool ChromeNativeAppWindowViewsAura::CanCloseWithInProgressDownloads() {
  ...
  if (dialog_type == Browser::DownloadCloseType::kOk)
    return true;
