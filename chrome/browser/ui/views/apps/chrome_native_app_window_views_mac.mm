...
views::NativeWidgetMac::TitleBarStyle ChromeNativeAppWindowViewsMac::title_bar_style() const {
  const NativeWidgetMac* widget_mac = reinterpret_cast<const NativeWidgetMac*>(widget());
  return widget_mac->title_bar_style();
}

void ChromeNativeAppWindowViewsMac::title_bar_style(views::NativeWidgetMac::TitleBarStyle style) {
  NativeWidgetMac* widget_mac = reinterpret_cast<NativeWidgetMac*>(widget());
...
void ChromeNativeAppWindowViewsMac::OnWidgetCreated(views::Widget* widget) {
  if (title_bar_style() != TitleBarStyle::NORMAL) {
    NativeWidgetMacNSWindow* window = (NativeWidgetMacNSWindow*)GetNativeWindow();
